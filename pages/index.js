...
export default function Home() {
  ...
  const [showPeekedCard, setShowPeekedCard] = useState(null);
  ...
  // Add effect to handle fire timer
  useEffect(() => {
    if (gameState?.isFireActive && gameState?.fireTimer) {
      const timer = setInterval(() => {
        if (gameState.fireTimer > 0) {
          setShowFireWarning(true);
        } else {
          setShowFireWarning(false);
        }
      }, 1000);
      return () => clearInterval(timer);
    }
  }, [gameState?.isFireActive, gameState?.fireTimer]);
